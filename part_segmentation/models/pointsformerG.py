"""
Based on PointsformerF, the dimension is identical for all stages, 256.
...
class get_model(nn.Module):
    def __init__(self, num_classes=50,points=2048, embed_dim=256, normal_channel=True,
        ...
        self.encoder_stage1 = encoder_stage(anchor_points=points//4, channel=256, reduce=True,
                                            pre_blocks=2, pos_blocks=2, k_neighbor=32)
        self.encoder_stage2 = encoder_stage(anchor_points=points//8, channel=256, reduce=True,
                                            pre_blocks=2, pos_blocks=2, k_neighbor=32)
        self.encoder_stage3 = encoder_stage(anchor_points=points // 16, channel=256, reduce=True,
                                            pre_blocks=2, pos_blocks=2, k_neighbor=32)
        self.encoder_stage4 = encoder_stage(anchor_points=points // 32, channel=256, reduce=True,
                                            pre_blocks=2, pos_blocks=2, k_neighbor=32)

        self.fp4 = PointNetFeaturePropagation(in_channel=256 + 256, mlp=[512,256,256])
        self.fp3 = PointNetFeaturePropagation(in_channel=256 + 256, mlp=[512, 256, 256])
        self.fp2 = PointNetFeaturePropagation(in_channel=256 + 256, mlp=[512, 256, 256])
        self.fp1 = PointNetFeaturePropagation(in_channel=256+128+128, mlp=[512, 256, 256])
