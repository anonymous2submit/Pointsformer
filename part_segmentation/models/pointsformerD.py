"""
Based on PointsformerC, add global context
Based on PointsformerA, two stages; and change pre_blocks, pos_blocks from 2 to 3
...
class get_model(nn.Module):
    def __init__(self, num_classes=50,points=2048, embed_dim=128, normal_channel=True,
        ...
        self.fuse_global_context = nn.Sequential(
            FCBNReLU1D(256*2*2, 512),
            FCBNReLU1D(512, 512)
        )

        self.fp2 = PointNetFeaturePropagation(in_channel=(256+128)*2, mlp=[512, 256])
    ...
    def forward(self, x, cls_label):
        ...
        global_context = F.adaptive_max_pool1d(fea_2, 1)
        fea_2 = torch.cat([fea_2, global_context.expand_as(fea_2)], dim=1)
        fea_2 = self.fuse_global_context(fea_2)

        l1_points = self.fp2(xyz_1, xyz_2, fea_1, fea_2)
