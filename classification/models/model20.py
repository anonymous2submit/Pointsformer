"""
Based on Model19, remove the dropout, keep GELU.
Based on Model14, add dropout to each block, use GELU replace ReLU
...
class FCBNReLU1DRes(nn.Module):
    def __init__(self, channel, kernel_size=1, dropout=0.1, bias=False):
...
class TransformerBlock(nn.Module):
    def __init__(self, dim, heads=8, dim_head=32, dropout=0.1, **kwargs):
        """
...
class Model20(nn.Module):
    def __init__(self, points=1024, class_num=40, embed_dim=64,
        ...
        # )
        self.embedding = nn.Conv1d(3, embed_dim, kernel_size=1, bias=False)
...
def model20A(num_classes=40, **kwargs) -> Model20:  # 20695MiB
    return Model20(points=1024, class_num=num_classes, embed_dim=64,
                  pre_blocks=[2, 2, 2], pos_blocks=[2, 2, 2], k_neighbors=[32, 32, 32],
                  reducers=[2, 2, 2], **kwargs)


def model20B(num_classes=40, **kwargs) -> Model20:  # 10817MiB
    return Model20(points=1024, class_num=num_classes, embed_dim=64,
                  pre_blocks=[2, 2, 2], pos_blocks=[2, 2, 2], k_neighbors=[32, 32, 32],
                  reducers=[4, 2, 2], **kwargs)


def model20C(num_classes=40, **kwargs) -> Model20:  # 25261MiB
    return Model20(points=1024, class_num=num_classes, embed_dim=64,
                 pre_blocks=[4,4,4], pos_blocks=[4,4,4], k_neighbors=[32,32,32],
                 reducers=[2,2,2], **kwargs)


def model20D(num_classes=40, **kwargs) -> Model20:  # 32305MiB
    return Model20(points=1024, class_num=num_classes, embed_dim=64,
                 pre_blocks=[2,2,2], pos_blocks=[2,2,2], k_neighbors=[48,48,48],
                 reducers=[2,2,2], **kwargs)


def model20E(num_classes=40, **kwargs) -> Model20:
    return Model20(points=1024, class_num=num_classes, embed_dim=64,
...
def model20Lite_A(num_classes=40, **kwargs) -> Model20:  # 7335MiB
    return Model20(points=1024, class_num=num_classes,embed_dim=64,
                 pre_blocks=[2,2], pos_blocks=[2,2], k_neighbors=[32,32],
                 reducers=[4,4], **kwargs)

def model20Lite_B(num_classes=40, **kwargs) -> Model20:  # 6003MiB
    return Model20(points=1024, class_num=num_classes,embed_dim=32,
                 pre_blocks=[2,2], pos_blocks=[2,2], k_neighbors=[32,32],
                 reducers=[4,4], **kwargs)


def model20Lite_C(num_classes=40, **kwargs) -> Model20:  # 7875MiB
    return Model20(points=1024, class_num=num_classes,embed_dim=64,
                 pre_blocks=[3,3], pos_blocks=[3,3], k_neighbors=[32,32],
                 reducers=[4,4], **kwargs)

def model20Lite_D(num_classes=40, **kwargs) -> Model20:  # 9767MiB
    return Model20(points=1024, class_num=num_classes,embed_dim=64,
                 pre_blocks=[3,3,3], pos_blocks=[3,3,3], k_neighbors=[32,32,16],
                 reducers=[4,4,2], **kwargs)

def model20Lite_E(num_classes=40, **kwargs) -> Model20:  # 6537MiB
    return Model20(points=1024, class_num=num_classes,embed_dim=32,
                 pre_blocks=[4,4], pos_blocks=[4,4], k_neighbors=[32,32],
                 reducers=[4,4], **kwargs)

def model20Lite_F(num_classes=40, **kwargs) -> Model20:  # 4969MiB
    return Model20(points=1024, class_num=num_classes,embed_dim=32,
                 pre_blocks=[2,3,2], pos_blocks=[2,3,2], k_neighbors=[32,16,8],
                 reducers=[4,4,4], **kwargs)


def model20Lite_G(num_classes=40, **kwargs) -> Model20:  # 7831MiB
    return Model20(points=1024, class_num=num_classes,embed_dim=48,
                 pre_blocks=[2,3,2], pos_blocks=[2,3,2], k_neighbors=[32,16,8],
                 reducers=[4,4,4], **kwargs)


def model20Lite_H(num_classes=40, **kwargs) -> Model20:  # 5745MiB
    return Model20(points=1024, class_num=num_classes,embed_dim=64,
