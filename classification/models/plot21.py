...
class Attention(nn.Module):
    def __init__(self, dim, heads = 8, dim_head = 32, dropout = 0.):
        ...
        self.register_buffer("attention_map", torch.rand(1))

    def set_attention_buffer(self, map):
    ...
    def forward(self, x):
        ...
        self.set_attention_buffer(attn)
...
class PreExtraction(nn.Module):
    ...
    def forward(self, x):
        ...
        # print(f"self.transformer.attention.attention_map.shape: {self.transformer.attention.attention_map.shape}")
        return x, self.transformer.attention.attention_map
...
class PosExtraction(nn.Module):
    ...
    def forward(self, x):  # [b, d, k]
        out = self.transformer(self.operation(x))
        return out, self.transformer.attention.attention_map


class Plot21(nn.Module):
    ...
    def forward(self, x):
        ...
        for i in range(self.stages):
            previous_xyz = xyz
            xyz, x, grouped_xyz = self.local_grouper_list[i](xyz, x.permute(0, 2, 1))   # [b,g,3]  [b,g,k,d]  [b,g,k,3]
            x, local_attention = self.pre_blocks_list[i](x)  # [b,d,g]
            x, global_attention = self.pos_blocks_list[i](x)  # [b,d,g]

            structure = {}
            structure["previous_xyz"] = previous_xyz
            structure["xyz"] = xyz
            structure["grouped_xyz"] = grouped_xyz
            structure["local_attention"] = local_attention
            structure["global_attention"] = global_attention
        ...
        x = self.classifier(x)
        return x, structure_list


def plot21H(num_classes=40, **kwargs) -> Plot21:
    return Plot21(points=1024, class_num=num_classes, embed_dim=128,
